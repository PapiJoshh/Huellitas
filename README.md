...
2. En GitHub ve a Settings > Pages y selecciona `GitHub Actions` como Source.
3. Publica `backend` en Render con `npm install` y `npm start`.
4. En Render configura las variables de `backend/.env.example`.
5. En GitHub ve a Settings > Secrets and variables > Actions > Variables y crea `VITE_API_URL` con la URL pública de Render.
...
La URL de este repositorio será `https://papijoshh.github.io/Huellitas/`.

### Seguridad

Nunca subas `backend/.env` ni `SUPABASE_SERVICE_ROLE_KEY` a GitHub. Si una clave privada apareció en un repositorio, revócala y genera otra inmediatamente desde el panel del proveedor.
